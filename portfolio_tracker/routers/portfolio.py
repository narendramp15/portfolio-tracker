...
async def list_portfolios(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """List all portfolios."""
...
async def create_portfolio(portfolio: schemas.PortfolioCreate, db: Session = Depends(get_db)):
    ...
    return crud.create_portfolio(db, name=portfolio.name, description=portfolio.description)
...
async def get_portfolio_assets(portfolio_id: int, db: Session = Depends(get_db)):
...
async def add_asset_to_portfolio(portfolio_id: int, asset: schemas.AssetCreate, db: Session = Depends(get_db)):
