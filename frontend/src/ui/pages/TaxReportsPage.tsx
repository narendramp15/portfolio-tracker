...
export default function TaxReportsPage() {
    ...
    const handleDownloadCSV = async () => {
        ...
        try {
            ...
            const response = await api.get(
                `/tax-reports/portfolios/${selectedPortfolioId}/capital-gains/export?${params}`,
                { responseType: 'blob' }
            )
            ...
            if (err?.response?.status === 402) {
                ...
                const detail = JSON.parse(text)?.detail || 'Upgrade to Pro for unlimited tax report exports.'
                setUpgradeReason(detail)
                setShowUpgrade(true)
                return
            }
            console.error('CSV export failed:', err)
            alert('Failed to export tax report CSV')
        }
    }
